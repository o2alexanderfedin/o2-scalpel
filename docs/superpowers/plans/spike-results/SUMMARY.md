...
| P5a | pylsp-mypy stale-rate | B — SHIP pylsp-mypy with documented cold-start warning (re-run 2026-04-26 reversed both falsifier axes; `solidlsp.decisions.p5a_mypy` is the canonical record) | Enable pylsp-mypy as a plugin inside pylsp-rope (`live_mode: false` + `dmypy: true`). basedpyright remains the cross-file type-error authority per MVP §11.1. Stale_rate 0.00% (0/12 on re-run); p95 2.668s. CHANGELOG note: first didSave after long idle may exceed 1s. |
