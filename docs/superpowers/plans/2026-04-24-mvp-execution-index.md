...
| **Stage 1A** | `2026-04-24-stage-1a-lsp-primitives.md` (TBD) | LSP primitive layer in `solidlsp` — codeAction/resolve/executeCommand wrappers, `$/progress` tracker, `applyEdit` reverse-request handler. Files 1–4 of §14.1, ~800 LoC | **Plan TBD** |
| **Stage 1B** | `2026-04-24-stage-1b-applier-checkpoints-transactions.md` (TBD) | `WorkspaceEdit` applier upgrade with full shape × options matrix; checkpoint/rollback machinery; transaction store. Files 5–7 of §14.1, ~830 LoC + ~80 unit tests for the applier matrix | **Plan TBD** |
...
| **Stage 1E** | `2026-04-24-stage-1e-language-strategies.md` (TBD) | `LanguageStrategy` Protocol + `RustStrategyExtensions` + `PythonStrategyExtensions` mixins; `RustStrategy` skeleton with assist-family declarations + extension whitelist; `PythonStrategy` skeleton with multi-server orchestration + 14-step interpreter discovery + Rope library bridge. Files 11–14 of §14.1, ~1,275 LoC | **Plan TBD** |
