...
**Status (2026-04-26): CLOSED** via `attach_apply_source` capture in `scalpel_facades.py` + `get_apply_source` helper in `facade_support.py`; all 6 spike sites migrated to deterministic helper. Investigation confirmed root cause was `linecache` instability (not decorator stacking — `apply` methods are not wrapped). 5 consecutive aggregate runs of all 6 spike files green (90/90 each); 5 consecutive runs of new 100-iteration regression test green (26/26 each). See `docs/superpowers/plans/2026-04-26-fix-inspect-getsource-flakes.md`.
...
- **rust-analyzer position validation** — _CLOSED 2026-04-26 (tag `stage-v0.2.0-followups-complete`, Leaf 02)._ `compute_file_range(path)` LSP helper at `vendor/serena/src/solidlsp/util/file_range.py` covers empty / single-line / multi-line / CRLF / lone-CR / missing-file. `RustAnalyzer.request_code_actions` `@override` adds preflight `ValueError` raise before any LSP round-trip on out-of-range positions. `whole_file_range` conftest fixture migrated to dual-mode (parametrized → helper; unparametrized → backwards-compat fallback). 12 tests (9 helper + 3 adapter); 0/0/0 pyright. Removes EOF-coordinate-math duplication from the 16 deferred Rust integration tests.
- **Multi-server async wrapping** — _CLOSED 2026-04-26 (tag `stage-v0.2.0-followups-complete`, Leaf 03)._ `serena.refactoring._async_check` ships `is_async_callable` + `assert_servers_async_callable`; `MultiServerCoordinator.__init__` now rejects raw sync servers with explicit `TypeError` pointing at `_AsyncAdapter`. Single-source-of-truth `AWAITED_SERVER_METHODS` constant feeds both the coordinator gate and `_AsyncAdapter._ASYNC_METHODS`. Real-adapter parallelism evidence test at `test/integration/test_multi_server_real_adapters_parallel.py` boots pylsp + basedpyright + ruff and asserts Amdahl-aware parallelism budget. 18 tests (11 helper + 6 init-validation + 1 integration); 0/0/0 pyright.
- **CARGO_BUILD_RUSTC workaround** — _CLOSED 2026-04-26 (tag `stage-v0.2.0-followups-complete`, Leaf 04)._ Inline `os.environ.setdefault("CARGO_BUILD_RUSTC", "rustc")` removed from 7 sites (1 conftest module-load + 6 spike modules). Workaround relocated to opt-in pytest plugin `vendor/serena/test/conftest_dev_host.py` gated on `O2_SCALPEL_LOCAL_HOST=1`; CI runs clean without the env override. Developer-facing shim at `scripts/dev_env_shim.sh`; single-source-of-truth doc at `docs/dev/host-rustc-shim.md`. 2 tests (subprocess-based, both branches).
